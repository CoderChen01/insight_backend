...
__all__ = ['FaceSettings', 'CameraExtractionSettings', 'AISkillSettings', 'CameraGroup', 'Camera']
...
class CameraExtractionSettings(models.Model):
    ...
    frequency = models.IntegerField(verbose_name='抽帧频率', choices=FREQUENCY_CHOICES)
...
class AISkillSettings(models.Model):
    """AI skill settings"""
    ai_skill = models.ForeignKey(verbose_name='AI技能绑定', to=AISkill, on_delete=models.CASCADE)
    coordinates = models.CharField(verbose_name='图像划分区域坐标', max_length=255)
    face_relevance = models.ForeignKey(verbose_name='人脸库关联(可选)',
                                          null=True, blank=True,
                                          to='FaceSettings', on_delete=models.CASCADE)
...
class CameraGroup(models.Model):
    ...
    description = models.CharField(verbose_name='分组描述', max_length=300, default='', blank=True)
...
class Camera(models.Model):
    ...
    user = models.ForeignKey(verbose_name='所属用户', to=User, on_delete=models.CASCADE)
    group = models.ForeignKey(verbose_name='所属分组', to='CameraGroup', on_delete=models.CASCADE)

    # AI skill settings
    ai_skill_settings = models.ManyToManyField(verbose_name='所用AI技能',
                                               blank=True,
                                               to='AISkillSettings')  # null has no effect on ManyToManyFild

    # extraction settings
    extraction_settings = models.OneToOneField(verbose_name='抽帧设置',
                                              null=True, blank=True,
    ...
    description = models.CharField(verbose_name='摄像头描述', max_length=300, default='', blank=True)
