...
class CreateCameraGroup(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        has_camera_group = CameraGroup.objects.filter(
            user=request.user, camera_group_id=camera_group_id
        ).first()

        if has_camera_group:
            retdata = BaseResponse(
                code=1, msg="success", data=1, success_msg="组ID已存在"
            )
            return Response(retdata.result)

        retdata = BaseResponse(
            code=1, msg="success", data=0, success_msg="组ID不存在，可新建"
        )
        return Response(retdata.result)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if has_camera_group:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="分组ID已存在，请重新设置"
            )
            return Response(retdata.result)
...
class RetrieveCameraGroup(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        retdata = BaseResponse(
            code=1, msg="success", success_msg="摄像头组数据请求成功", data=data.data
        )
...
class CreateCamera(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        has_camera = Camera.objects.filter(
            user=request.user, camera_id=camera_id
        ).first()

        if has_camera:
            retdata = BaseResponse(
                code=1, msg="success", data=1, success_msg="摄像头ID已存在"
            )
            return Response(retdata.result)

        retdata = BaseResponse(
            code=1, msg="success", data=0, success_msg="摄像头ID不存在，可新建"
        )
        return Response(retdata.result)
    ...
    def post(self, request, *args, **kwargs):
        ...
        has_camera = Camera.objects.filter(
            user=request.user, camera_id=serializer.validated_data["camera_id"]
        ).first()
        has_camera_group = CameraGroup.objects.filter(
            user=request.user,
            camera_group_id=serializer.validated_data["camera_group_id"],
        ...
        if has_camera:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="摄像头ID已存在，请重新设置"
            )
            return Response(retdata.result)

        if not has_camera_group:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="摄像头分组不存在，不可绑定到该分组"
            )
            return Response(retdata.result)
...
class RetrieveCamera(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        cameras = Camera.objects.filter(
            group__camera_group_id=serializer.validated_data["camera_group_id"]
        )
        cameras_serializer = RetrieveCameraSerializer(
            instance=cameras, many=True
        )
...
class UpdateCamera(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if camera_obj.state == 21:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="无法修改摄像头，已绑定技能或者正在运行"
            )
            return Response(retdata.result)
...
class DeleteCamera(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not camera_obj:
            retdata = BaseResponse(code=0, msg="error", error_msg="没有该摄像头")
            return Response(retdata.result)
        ...
        if camera_obj.state in (13, 20, 21, 22):
            if serializer.validated_data["force"]:
                ...
                retdata = BaseResponse(
                    code=0, msg="error", error_msg="无法删除摄像头，已绑定技能或者正在运行"
                )
                return Response(retdata.result)

        camera_obj.delete()
        retdata = BaseResponse(code=1, msg="success", success_msg="删除摄像头成功")
        return Response(retdata.result)
...
class SetExtractFrameSettings(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if camera.state == 21:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="摄像头正在运行中，无法设置抽帧配置"
            )
            return Response(retdata.result)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not serializer.is_valid():
            ...
            if serializer.errors.get("camera_id"):
                ...
                if (
                    serializer.errors["ai_skill_settings"][0].code
                    == "required"
                ):
                    ...
                    retdata.error_msg = serializer.errors["ai_skill_settings"][
                        0
                    ]
            return Response(retdata.result)
        ...
        if not camera_obj.extraction_settings:
            retdata = BaseResponse(code=0, msg="error", error_msg="请先配置抽帧设置")
            return Response(retdata.result)

        if camera_obj.state == 21:
            retdata = BaseResponse(
                code=0, msg="error", error_msg="摄像头正在运行中，无法设置AI技能配置"
            )
            return Response(retdata.result)
        ...
        for ai_skill_setting in ai_skill_settings:
            has_ai_skill = AISkill.objects.filter(
                user=request.user, ai_skill_id=ai_skill_setting["ai_skill_id"]
            ).first()
            if not has_ai_skill:
                retdata = BaseResponse(
                    code=0, msg="error", error_msg="配置中含有不存在的AI技能"
                )
                return Response(retdata.result)
            ...
            if face_relevance:
                ...
                for face_group_id in face_groups_id:
                    ...
                    if not has_face_group:
                        retdata = BaseResponse(
                            code=0, msg="error", error_msg="配置中含有不存在的人脸库"
                        )
                        return Response(retdata.result)
...
class Start(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not extraction_settings:
            retdata = BaseResponse(code=0, msg="error", error_msg="请配置抽帧设置")
            return Response(retdata.result)

        if not ai_skill_settings:
            retdata = BaseResponse(code=0, msg="error", error_msg="请配置AI技能配置")
            return Response(retdata.result)

        task_id = request.user.username + "##" + camera_obj.camera_id
        old_periodic_task = PeriodicTask.objects.filter(name=task_id).first()
        ...
        task_id = (
            ...
            + "##"
            + serializer.validated_data["camera_id"]
        )
