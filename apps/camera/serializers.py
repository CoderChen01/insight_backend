...
class HasCameraGroupSerializer(serializers.Serializer):
    camera_group_id = serializers.CharField(
        max_length=50,
        validators=[
            id_validator,
        ],
...
class CreateCameraGroupSerializer(serializers.ModelSerializer):
    class Meta:
        model = CameraGroup
        fields = ("camera_group_id", "name", "description", "add_time")
...
class RetrieveCameraGroupSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {"add_time": {"format": "%Y-%m-%d %H:%M:%S"}}


class UpdateCameraGroupSerializer(serializers.ModelSerializer):
    class Meta:
        model = CameraGroup
        fields = ("name", "camera_group_id", "description")
        extra_kwargs = {
...
class CreateCameraSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        return Camera.objects.create(
            state=11, group=camera_group, **validated_data
        )
...
class AISkillSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        read_only_fields = fields
        extra_kwargs = {"state": {"source": "get_state_display"}}
...
class FaceSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FaceSettings
        fields = ("face_group", "similarity", "quality")
        read_only_fields = fields
...
class AISkillSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("ai_skill", "coordinates", "face_relevance")
        read_only_fields = fields
...
class ExtractionSettingsSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("frequency", "start_time", "end_time")
        read_only_fields = fields
...
class UpdateCameraSerializer(serializers.ModelSerializer):
    class Meta:
        model = Camera
        fields = ("camera_id", "name", "description", "camera_url")
        extra_kwargs = {
            "camera_id": {
                "validators": [
                    id_validator,
...
class SetExtractFrameSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CameraExtractionSettings
        fields = ("camera_id", "frequency", "start_time", "end_time")
        extra_kwargs = {
    ...
    def create(self, validated_data):
        ...
        extraction_settings = CameraExtractionSettings.objects.create(
            **validated_data
        )
...
class SetAISkillSettingsSerializer(serializers.Serializer):
    ...
    ai_skill_settings = serializers.ListField(
        child=serializers.JSONField(), max_length=25, allow_empty=True
    )

    @staticmethod
    def validate_face_relevance(face_relevance):
        face_fields = ("face_groups_id", "similarity", "quality")
    ...
    def validate_coordinates(coordinates):
        ...
        for coordinates_field in coordinates:
            if coordinates_field not in coordinates_fields:
                raise serializers.ValidationError(
                    "图像分析区域存在多于参数" + coordinates_field
                )
        ...
        if (
            coordinates[coordinates_fields[2]]
            <= coordinates[coordinates_fields[0]]
        ):
            raise serializers.ValidationError("x_max应该大于x_min")

        if (
            coordinates[coordinates_fields[3]]
            <= coordinates[coordinates_fields[1]]
        ):
    ...
    def validate_ai_skill_settings(self, value):
        ...
        fields = ("ai_skill_id", "face_relevance", "coordinates")
        ...
        for ai_skill_setting in value:
            ...
            if fields[0] not in ai_skill_setting:
                ...
                try:
                    ...
                    raise serializers.ValidationError(
                        "AI技能ID不合法，为6-20为字母、数字或下划线"
                    )

            face_relevance = ai_skill_setting.get(fields[1])
            if face_relevance:
                self.validate_face_relevance(face_relevance)
    ...
    def create(self, validated_data):
        ...
        camera_obj = Camera.objects.filter(
            user=user, camera_id=validated_data.pop("camera_id")
        ).first()
        ...
        if ai_skill_settings:
            ...
            for ai_skill_setting in ai_skill_settings:
                ai_skill = AISkill.objects.filter(
                    user=user, ai_skill_id=ai_skill_setting["ai_skill_id"]
                ).first()
                ai_skill.bound = True
                ai_skill.save()
                ...
                if face_relevance:
                    ...
                    for face_group_id in face_relevance["face_groups_id"]:
                        face_group = FaceGroup.objects.filter(
                            user=user, face_group_id=face_group_id
                        ).first()
