...
from .models import FaceGroup, Face
...
class FaceGroupIDSerialzier(serializers.Serializer):
    face_group_id = serializers.CharField(max_length=50, validators=[id_validator, ])


class FaceIDSerializer(serializers.Serializer):
    face_id = serializers.CharField(max_length=50, validators=[id_validator, ])
...
class FaceGroupSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        face_group = instance.filter(face_group_id=validated_data.pop('face_group_id')).first()
        face_group.name = validated_data.get('name', face_group.name)
        face_group.description = validated_data.get('description', face_group.description)
        face_group.save()
        return face_group
