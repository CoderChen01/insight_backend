...
class AISkillGroup(models.Model):
    ...
    description = models.CharField(verbose_name='分组描述', max_length=300, default='', blank=True, null=True)
...
class AISkill(models.Model):
    ...
    group = models.ForeignKey(verbose_name='所属分组', to='AISkillGroup', on_delete=models.CASCADE)
    name = models.CharField(verbose_name='接口名称', max_length=30)
    description = models.CharField(verbose_name='接口描述', max_length=300, default='', blank=True, null=True)
    add_time = models.DateTimeField(verbose_name='最后修改时间', auto_now=True)
