...
class InterfaceIDSerializer(serializers.Serializer):
    ai_skill_id = serializers.CharField(
        max_length=50,
        validators=[
            id_validator,
        ],
...
class InterfaceGroupSerializer(serializers.ModelSerializer):
    class Meta:
        model = AISkillGroup
        fields = ("ai_skill_group_id", "name", "description", "add_time")
        extra_kwargs = {
            "ai_skill_group_id": {
                "validators": [
                    id_validator,
...
class InterfaceSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        return AISkill.objects.create(
            group=ai_skill_group, state=1, **validated_data
        )
...
class UpdateInterfaceSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ai_skill = instance.filter(
            ai_skill_id=validated_data.pop("ai_skill_id")
        ).first()
        ai_skill.name = validated_data.get("name", ai_skill.name)
        ai_skill.description = validated_data.get(
            "description", ai_skill.description
        )
