...
class ExtendedTokenObtainSerializer(TokenObtainSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[self.username_field] = serializers.CharField(
            required=False, validators=[username_validator,]
        )
    ...
    def validate(self, attrs):
        ...
        if username and not email:
           ...
           self.user = _login_by_email_authenticate(email, attrs['password'])
            ...
            raise serializers.ValidationError('邮箱或用户名不能同时传入')  # represent validation failed
        else:
            raise serializers.ValidationError('账号必须传入')

        if self.user is None or not self.user.is_active:
            raise serializers.ValidationError('账号或者密码错误')  # represent validation failed
